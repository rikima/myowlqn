...
/**
 * minimizer implement class via OWLQN
 * 
 * 
 * 
 * @author rikitoku
...
public class OWLQNState implements Minimizable {
    ...
    static double C1 = 1e-6;
    static int MAX_ITER = 100;
    static double TOL = 1.0e-6;
    
    /*
    static {
        boolean verbose = Logreg.verbose;
    }
    */
     ...
     * 
     */
    public OWLQNState(LossFunction lossFunction, MLData mldata, double c) {
        this.mldata = mldata;
    ...
    public FeatureVector minimize() {
        ...
        stderr.println("# minimize time:" + tt + "[ms]");
    ...
    private double dirDeriv() {
    	if (C == 0) {
    		return ArrayUtils.dot(dir, grad);
        } 
        else {
            double val = dir[0] * grad[0];
            for (int i = 1; i < dim; ++i) {
                if (dir[i] != 0) { 
                    if (w[i] < 0) {
                        val += dir[i] * (grad[i] - C);
